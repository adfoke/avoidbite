...
`./build/avoidbite N` 各连跑取均值（macOS arm64；N=0 跑 3 次，其余 5 次；耗时为程序自报，`叮N口` 为平均总叮咬）：

```
蚊子数   平均离坑耗时（1 格 = 0.25s）
  0  █████████████████████████████████▍            8.35s  叮  0口
  1  ████████▌                                     2.15s  叮 23口
  2  █████                                         1.26s  叮 28口
  3  ███▋                                          0.94s  叮 30口
  5  ██▋                                           0.67s  叮 33口
  8  ██                                            0.50s  叮 35口
 12  ██▍                                           0.59s  叮 64口   <- 反超 8 只：过密互扰 + 锁竞争
 16  ██▎                                           0.58s  叮 84口
 24  █▊                                            0.45s  叮 96口
 32  █▎                                            0.33s  叮 97口
 48  █                                             0.25s  叮 98口
